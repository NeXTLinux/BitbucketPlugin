...
define('BitbucketPlugin/PullRequestUIOverview', [
        'jquery',
        ...
        function getScanStatusPoller() {
            server.rest({
                url: navbuilder.rest("bitbucketplugin").addPathComponents("prscan", "result", state.getProject().key,
                ...
                success: function (results, textStatus, jqXHR) {
                    ...
                    if (jqXHR.status === 200) {
                        ...
                        if (results["secretScanStatus"] === "COMPLETED") {
                            console.log("BitbucketPlugin scan result has been retrieved successfully! Updating UI elements");
                            updateOverviewLinkWithResults(results);
                        } else if (results["secretScanStatus"] === "IN_PROGRESS") {
                            console.log("BitbucketPlugin scan is still inprogress. Will recheck in " + CHECK_INTERVAL + "ms")
                        } else {
                            console.log("BitbucketPlugin getScanStatus unknown or failed! Status: " + jqXHR.status + " results:" + results);
                ...
                error: function (jqXHR, textStatus, errorThrown) {
                    if (jqXHR.status === 404) {
                        console.log("BitbucketPlugin scan has still not started. Will recheck in " + CHECK_INTERVAL + "ms")
                    } else {
                        console.log("BitbucketPlugin getScanStatus failed! Status: " + jqXHR.status + " Error:" + errorThrown.toString());
                        updateOverviewLinkAsFailed();
                    }
                },
                statusCode: {
        ...
        function updateOverviewLinkWithResults(scanResult) {
            ...
            var linkEle = $(".bitbucketplugin-overview-link");
            var labEle = $(".bitbucketplugin-overview-link .label");
        ...
        function updateOverviewLinkAsFailed() {
            ...
            labEle.text("BitbucketPlugin secret scan failed");
        }
        ...
        function setOnClickHandler(foundSecrets, prurl) {
            ...
            $(document).on('click', '.bitbucketplugin-overview-link .label', function (e) {
                e.preventDefault();

                if (foundSecrets.length > 0)
                    var dialog = AJS.dialog2($(com.cyanoth.bitbucketplugin.overviewDialog({foundSecrets: foundSecrets, prurl: prurl})));
                else
                    var dialog = AJS.dialog2($(com.cyanoth.bitbucketplugin.noSecretsDialog()));
