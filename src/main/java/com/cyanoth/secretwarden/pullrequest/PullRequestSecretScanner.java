...
import com.cyanoth.bitbucketplugin.collections.MatchRuleSet;
import com.cyanoth.bitbucketplugin.SecretScanException;
import com.cyanoth.bitbucketplugin.SecretScanStatus;
import com.cyanoth.bitbucketplugin.SecretScanner;
import com.cyanoth.bitbucketplugin.config.MatchRuleSetCache;
...
public class PullRequestSecretScanner implements SecretScanner {
    ...
    public PullRequestSecretScanResult scan(Boolean force) throws SecretScanException {
        log.debug(String.format("BitbucketPlugin scanning pull request: %s", prString()));
        ...
        try {
            ...
            if (scan != null) {
                if (scan.getPullRequestLastUpdated().compareTo(pullRequest.getUpdatedDate()) == 0) {
                    ...
                    log.debug(String.format("BitbucketPlugin has already scanned the pull request: %s but the information is out of date." +
            ...
            if (scanLock != null && scanLock.tryLock()) {
                try {
                    log.debug(String.format("BitbucketPlugin has obtained a clusterlock to scan the pull-request %s", prString()));
                ...
                log.info(String.format("BitbucketPlugin failed to obtain a clusterlock to scan the pull-request %s (another node scanning?)",
                        prString()));
                scan = checkOngoingScan(pullRequest);
            }
    ...
    private PullRequestSecretScanResult doSecretScan(PullRequest pullRequest) {
        ...
        log.debug(String.format("BitbucketPlugin is performing a scan on pull-request %s", prString()));
        ...
        log.info(String.format("BitbucketPlugin scanned the pull request: %s and has found %d secret(s).",
                prString(), scanResult.countFoundSecrets()));

        if (log.isDebugEnabled()) {
            long elapsedTime = System.currentTimeMillis() - scanStartTime;
            log.debug(String.format("BitbucketPlugin took %d milliseconds to scan the pull request: %s", elapsedTime, prString()));
        }
    ...
    private PullRequestSecretScanResult checkOngoingScan(PullRequest pullRequest) throws SecretScanException {
        ...
        try {
            while (curAttempts <= MAX_ATTEMPTS) {
                scan = getCachedSecretScan(pullRequest);
                if (scan == null)
                    throw new Exception(String.format("BitbucketPlugin couldn't find the cached entry for PR: %s", prString()));

                SecretScanStatus status = scan.getSecretScanStatus();

                if (status == SecretScanStatus.IN_PROGRESS) {
                    if (curAttempts % 10 == 0)
                        log.debug(String.format("BitbucketPlugin is reportedly still scanning the pull request: %s", prString()));
                    ...
                    log.debug(String.format("BitbucketPlugin had previously attempted to scan a pull request: %s but failed",
                            prString()));
                    break;
                } else {
                    log.debug(String.format("BitbucketPlugin has already scanned the pull request: %s and found %d secret(s).",
            ...
            log.error("BitbucketPlugin failed to get the scan results from cache!");
            throw new SecretScanException(e);
        }
    ...
    public Lock getScanLock(Object scope) {
        ...
        String lockKey = String.format("com.cyanoth.bitbucketplugin.prscan.lock_%d_%d",
