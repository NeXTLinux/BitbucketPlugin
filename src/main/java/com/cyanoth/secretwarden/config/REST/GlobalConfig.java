package com.cyanoth.bitbucketplugin.config.REST;
...
import com.cyanoth.bitbucketplugin.config.MatchRuleSettings;
import com.cyanoth.bitbucketplugin.pullrequest.PullRequestSecretScanResultCache;
import com.cyanoth.bitbucketplugin.structures.MatchRule;
...
public class GlobalConfig  {
    ...
    public Response clearResultCache() {
        try {
            this.permissionValidationService.validateForGlobal(Permission.ADMIN);
            try {
                log.debug("Clearing BitbucketPlugin Result Cache...");
                pullRequestSecretScanResultCache.clear();
                return Response.ok("The secret result cache has been cleared!").build();
            }
            catch (Exception e) {
                log.error("Failed to clear BitbucketPlugin result cache! An error occurred.", e);
    ...
    public Response reloadRuleSet() {
        try {
            ...
            try {
                ...
                return Response.ok("BitbucketPlugin MatchSecretRuleSet reloaded!").build();
            }
            catch (Exception e) {
                log.error("Failed to reload BitbucketPlugin ruleset. An error occurred.", e);
                return Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(new RestErrorMessage("The BitbucketPlugin MatchRuleSet has NOT been reloaded." +
                        "An error occurred, see server-logs for more information")).build();
            }
