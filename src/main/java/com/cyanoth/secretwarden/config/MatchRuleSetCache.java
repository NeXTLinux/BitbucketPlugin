package com.cyanoth.bitbucketplugin.config;
...
import com.cyanoth.bitbucketplugin.RuleSetLoadException;
...
public class MatchRuleSetCache {
    ...
    private final String CACHE_NAME = "com.cyanoth.bitbucketplugin:MatchRuleSetCache";
    ...
    private Cache<String, MatchRuleSet> cache() {
        synchronized (this) {
            if (this._matchRuleSet == null) {
                ...
                try {
                    reloadRuleSet();
                    log.debug("BitbucketPlugin: A new cache object has been initialised for MatchRuleSet!");
                }
                catch (RuleSetLoadException e) {
                    log.error("Failed to load BitbucketPlugin RuleSet! RuleSet may be empty...");
    ...
    public void reloadRuleSet() throws RuleSetLoadException {
        log.info("Reloading BitbucketPlugin MatchRuleset");
        try {
            ...
            log.info(String.format("BitbucketPlugin ruleset reloaded successfully and contains: %d rules", cache().get(RULESET_KEY).count()));
        }
        catch (RuleSetLoadException e) {
            log.error(String.format("Failed to reload BitbucketPlugin ruleset.\nAn exception has occurred: %s", e.getMessage()));
            throw e;
        }
    }
